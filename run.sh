...
This may take a moment while the Shipyard images are pulled..."
    ...
    sleep 2
    echo "Starting DB..."
    db=$(docker -H unix://docker.sock run -i -t -d -p 5432 -e POSTGRESQL_DB=shipyard -e POSTGRESQL_USER=shipyard -e POSTGRESQL_PASS=$DB_PASS -name shipyard_db shipyard/postgresql)
    sleep 5
